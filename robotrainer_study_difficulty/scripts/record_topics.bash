...
TOPICS_YAML=$(rospack find robotrainer_study_difficulty)/config/topics_to_record.yaml
CONFIG_YAML=$(rospack find robotrainer_study_difficulty)/config/user_study_manager.yaml
STATUS_FILE=$(rospack find robotrainer_study_difficulty)/status/user_study_manager.status
...
# The command inside flock runs in a subshell, so we pipe its output to the parent's `readarray`.
readarray -t STATUS < <(flock -s "$STATUS_FILE" cat "$STATUS_FILE")
